...
#   Samuele ~redShadow~ Santi (redshadow@hackzine.org)
#
...
def main():
    ...
    optparser.add_option(
        ...
        action='store_true',
        default=False,
        help='disable the gloss effect applied to covers')
    optparser.add_option(
        '--volume-button',
        action='store_true',
        default=False,
        help='Always show the volume button')
    (options, args) = optparser.parse_args()

    ## Check whether DAP is already running?
    ## todo: check for lockfile in settings, more reliable than this!
    if not options.multiple_instances:
        shouldStop  = False
        dbusSession = None
        ...
        try:
            ...
            if consts.dbusService in activeServices:
                ...
                dbus.Interface(
                    dbusSession.get_object(consts.dbusService, '/'),
                ...
                if len(args) != 0:
                    dbus.Interface(
                        dbusSession.get_object(consts.dbusService, '/TrackList'),
                        consts.dbusInterface
                        ).SetTracks(args, True)
        except:
            ## why??
    ...
    logger.info('Decibel started')
    prefs.setCmdLine((options, args)) ## why??
    ...
    gtk.window_set_default_icon_list(*[
        gtk.gdk.pixbuf_new_from_file(k)
            for k,v in consts.APP_ICONS.iteritems()
    ])
    ...
    try:
        ...
        logger.info('No RGBA support (requires PyGTK 2.10+)')
    ...
    def delayedStartup():
        """
        Perform all the initialization stuff that is not mandatory
        to display the window.
        This function should be called within the GTK main loop,
        ...
        ## If this is the first time the application is started, show
