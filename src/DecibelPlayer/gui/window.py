...
class Window(gtk.Window):
    ...
    def __init__(self, resFile, container, modName, title, defaultWidth, defaultHeight):
        ...
        if prefs.get(modName, 'win-is-maximized', False):
            self.maximize()
        self.resize(prefs.get(modName, 'win-width', defaultWidth), prefs.get(modName, 'win-height', defaultHeight))
    ...
    def onResize(self, win, rect):
        ...
        if win.window is not None and not win.window.get_state() & gtk.gdk.WINDOW_STATE_MAXIMIZED:
            prefs.set(self.modName, 'win-width',  rect.width)
            prefs.set(self.modName, 'win-height', rect.height)


    def onState(self, win, evt):
        """ Save the new state of the dialog """
        prefs.set(self.modName, 'win-is-maximized', bool(evt.new_window_state & gtk.gdk.WINDOW_STATE_MAXIMIZED))
